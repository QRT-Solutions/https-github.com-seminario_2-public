...
def test_presentation_uses_local_required_resources():
    html = (ROOT / "masterclass/index.html").read_text()
...
def test_vendor_bytes_have_identified_licenses():
    manifest = json.loads((ROOT / "masterclass/vendor/manifest.json").read_text())
...
def test_keyboard_and_motion_accommodations_are_present():
    html = (ROOT / "masterclass/index.html").read_text()
    css = (ROOT / "masterclass/styles.css").read_text()
    js = (ROOT / "masterclass/app.js").read_text()
...
def test_deferred_tracks_are_absent_from_masterclass():
    ...
    for name in ("index.html", "app.js", "guion.md"):
        assert not re.search(r"\b(?:NQ|GC)\b|Nasdaq|XAUUSD", (masterclass / name).read_text(), re.I)
